...
describe('/covid/country/name', () => {
    test('Test covid endpoint with query parameter', async () => {
        ...
        const covidRes = await request(app).get('/covid/country/name?country=' + country).set({ accept: 'application/json', authorization: 'Bearer ' + res.body.token });
...
describe('/covid/country/name', () => {
    test('Test covid endpoint with path parameter', async () => {
        ...
        const res = await request(app).post('/users/signin').send(data);
        const covidRes = await request(app).get('/covid/country/name/' + country).set({ accept: 'application/json', authorization: 'Bearer ' + res.body.token });
