...
const countryInPath = async (req, res) => {
    try {
      ...
      mailService.sentMail(req.userData.mail+' has accessed covid route with path parameters for country ' + country);
...
const countryInQuery = async (req, res) => {
  try {
    ...
    mailService.sentMail(req.userData.mail+' has accessed covid route with query parameters for country ' + country);
