const getRequests = (req, res) => {
  res.status(200).json({
    ...
    description:
      'The GET method is used to retreive data from a server at the specified resource.Since a GET request is only requesting data and not modifying any resources, its considered a safe and idempotent method.GET reqs are read only while others write something on server',
  });
};
...
const postRequests = (req, res) => {
  try {
    ...
    res.status(400).json({
      message:
        'Please provide your info in JSON format correctly for name and description',
    });
  }
};
...
const patchRequests = (req, res) => {
  res.status(200).json({
    ...
    description:
      'PATCH requests are to make partial update on a resource. Also PUT requests modifys a resource entity, so to make more clear – PATCH method is the correct choice for partially updating an existing resource, and PUT should only be used if you’re replacing a resource in its entirety.',
  });
};
